...
@router.message(Command('my_car_blockes'))
async def is_blocked(message: types.Message, state: FSMContext):
    await message.answer('Введите регистрационный номер машины, которую ВЫ подперли. ')
    await state.set_state(MyCarBlockes.license_plate)


@router.message(MyCarBlockes.license_plate)
async def enter_license_plate(message: types.Message, state: FSMContext):
    ...
    if owners:
        ...
        await message.answer(
            'Сообщить владельцу о том, что ВЫ его подперли?!',
            reply_markup=kb_is_blocked.keyboard_is_blocked
        )
        await state.set_state(MyCarBlockes.answer)
    else:
        await message.answer('К сожалению, владельцев данного автомобиля не найдено :( ')


@router.message(MyCarBlockes.answer)
...
async def writing_message(username, state: FSMContext):
    ...
    if car_info:
        for brand_car, license_plate in car_info:
            text = \
                f"Вас подпер автомобиль марки: {brand_car}, \n"\
                f"Регистрационный номер: {license_plate}, \n"\
                f"Просим не беспокоиться!, \n"\
                f"Если необходимо уехать - звоните владельцу!"
